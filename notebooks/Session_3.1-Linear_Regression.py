...
@app.cell
def _(RANDOM_SEED, fish_train, pm):
    def build_baseline():
...
@app.cell
def _(az, baseline_idata, unpooled_idata):
...
def _(
    ...
    n_outliers = 5
    st_outlier_species = rng_st.choice(fish_train["Species"].unique(maintain_order=True).to_list(), n_outliers)
    ...
    st_species_idx_aug = (
        pl.Series(st_fish_train_aug["Species"]).cast(pl.Categorical).to_physical().to_numpy()
    )
    st_species_names_aug = st_fish_train_aug["Species"].unique(maintain_order=True).sort().to_list()
    st_aug_coords = {
        "slopes": ["width_effect", "height_effect", "length_effect"],
    ...
    def st_build_normal():
        with pm.Model(coords=st_aug_coords) as model:
            lw = pm.Data("log_width", (st_fish_train_aug["log_width"] - train_mean_log_width).to_numpy(), dims="obs_idx")
            lh = pm.Data("log_height", (st_fish_train_aug["log_height"] - train_mean_log_height).to_numpy(), dims="obs_idx")
            ll = pm.Data("log_length", (st_fish_train_aug["log_length"] - train_mean_log_length).to_numpy(), dims="obs_idx")
            lweight = pm.Data("log_weight", st_fish_train_aug["log_weight"].to_numpy(), dims="obs_idx")
            ...
            expected = mu[s] + beta[0, s] * lw + beta[1, s] * lh + beta[2, s] * ll
            sigma = pm.HalfNormal("sigma", 1.0)
            pm.Normal("log_obs", mu=expected, sigma=sigma, observed=lweight, dims="obs_idx")
...
def _(
    ...
    def st_build_studentt():
        with pm.Model(coords=st_aug_coords) as model:
            ...
            pm.StudentT("log_obs", nu=nu, mu=expected, sigma=sigma, observed=lweight, dims="obs_idx")
...
@app.cell
def _(az, st_normal_idata, st_studentt_idata):
    st_comparison = az.compare(
        {"Normal (augmented)": st_normal_idata, "Student-T (augmented)": st_studentt_idata}
    )
    st_comparison
    return
...
def _(mo):
    mo.accordion(
        ...
        intercept_prior = pz.maxent(pz.Normal(), lower=1.6, upper=7.4, mass=0.9, plot=False)
        slope_prior = pz.maxent(pz.Normal(), lower=0.0, upper=3.0, mass=0.9, plot=False)
        sigma_prior = pz.maxent(pz.HalfNormal(), lower=0.0, upper=1.0, mass=0.9, plot=False)

        species_list = fish_train["Species"].unique(maintain_order=True).sort().to_list()
        ...
        with pm.Model(coords=coords) as improved_model:
            ...
            pm.sample_posterior_predictive(
                improved_idata, extend_inferencedata=True, random_seed=RANDOM_SEED
            )
...
@app.cell
def _(fish_test, np, oos_pred_weights, pl):
...
def _(np, oos_pred_weights):
    ...
    cost_matrix
    return optimal_tier, tier_edges
...
@app.cell
def _(fish_test, np, oos_pred_weights, optimal_tier, pl, tier_edges):
...
def _(mo):
    mo.accordion(
        ...
        n_changed, n_upward, new_optimal_tier
        ```
...
def _(mo):
    ...
    This workflow applies to every model we build, from simple regression to the GLMs we'll cover next and the hierarchical models in Session 4. The key discipline is to **always check before you trust**: priors before fitting, diagnostics before interpreting, posterior predictions before concluding, and a stated loss function before declaring a decision.
    """)
    return
