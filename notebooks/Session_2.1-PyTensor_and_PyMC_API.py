...
def _():
    mo.md(r"""
    Just for fun, let's add an `pytensor.tensor.exp` before the `pytensor.tensor.log` (to get the identity function).
    """)
    return
...
def _(w):
    ...
    return new_parent_of_w, parent_of_w
...
def _():
    mo.md(r"""
    To modify the graph we need to use the `pytensor.graph.replace.clone_replace` function, which _returns a copy of the initial subgraph with the corresponding substitutions._
    """)
    return
...
@app.cell
def _(new_w, x, y):
    f_1 = pytensor.function(inputs=[x, y], outputs=new_w)
    f_1.dprint()
    return
...
def _():
    ...
    While some rewrites are performed automatically, we can invoke them manually as well.
...
@app.cell
def _(y_1):
    stable_y = pytensor.graph.rewrite_graph(y_1, include=("stabilize",))
    stable_y.dprint()
    return
...
def _():
    ...
    Let's try building a simple statistical model in pure PyTensor. We will estimate the parameters of a logistic regression model using a simple version of **gradient descent**.
    ...
    where $x$ corresponds to the `log_dose` and $y$ is the number of `deaths`.
    """)
    return
...
def _():
    ...
    a_1 = pt.tensor("a", shape=())
    b_1 = pt.tensor("b", shape=())
    return X, Y, a_1, b_1
...
def _(X, Y, a_1, b_1, n):
    ...
    cost = xent.mean()
    stable_cost = pytensor.graph.rewrite_graph(
        cost, include=("canonicalize", "stabilize")
    )
    ga, gb = pt.grad(stable_cost, [a_1, b_1])
    ...
    step_new = step * 0.99
    return a_new, b_new, cost, prediction, step, step_new, xent
...
def _():
    mo.md(r"""
    Recall that in order to use these expressions, we need to **compile** them into functions--nothing has been executed yet!
...
@app.cell
def _(X, Y, a_1, a_new, b_1, b_new, prediction, step, step_new, xent):
    train = pytensor.function(
        inputs=[X, Y, a_1, b_1, step],
        outputs=[prediction, xent, a_new, b_new, step_new],
    )
    predict = pytensor.function(inputs=[X, a_1, b_1], outputs=prediction)
    return (train,)
...
@app.cell(hide_code=True)
def _(deaths, dose, train):
    def run_gradient_descent():
        a, b, lr = (0.0, 1.0, 10.0)
        training_steps = 1000
        for i in range(training_steps):
            pred, err, a, b, lr = train(dose, deaths, a, b, lr)
        return a, b, lr

    alpha, beta, final_lr = run_gradient_descent()
    _output = mo.md(
        f"**Final model parameters:** intercept = {alpha:.4f}, slope = {beta:.4f}, learning rate = {final_lr:.6f}"
    )
...
@app.cell(hide_code=True)
def _(alpha, beta, deaths, dose, n):
    def logit(x):
